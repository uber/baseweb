...
const source = resolve(__dirname, '../documentation-site/pages/examples');
...
for (let postDir of postDirs) {
  ...
  let post = {
    path: `/examples/${basename(postDir)}`,
    ...attrs,
  };
  posts.push(post);
}
...
writeFileSync(`${process.cwd()}/documentation-site/showcase.js`, content);
