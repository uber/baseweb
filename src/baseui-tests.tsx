...
const NewStyledProgressSteps = styled<
  ProgressStepsStyleProps,
  typeof StyledProgressSteps
>(StyledProgressSteps, ({$theme, $disabled}) => ({marginTop: '10px'}));

<StyledAnchor href="url" />;
<NewStyledProgressSteps $disabled="string" />; // $ExpectError
<NewStyledProgressSteps $disabled={false} />;

<BaseProvider theme={newTheme} />; // $ExpectError

// Accordion
<Accordion>
  <Panel overrides={{Header: {}}} title="Accordion panel 1">
    Content
  </Panel>
</Accordion>;

<Accordion>
  <Panel overrides={{InvalidOverride: {}}} title="Accordion panel 1">
    Content
  </Panel>{' '}
  // $ExpectError
</Accordion>;
...
<StatefulButtonGroup mode="dropdown" initialState={{selected: 0}}>
  {' '}
  // $ExpectError
  <Button>Label</Button>
  <Button>Label</Button>
...
<StatefulCheckbox onChange={console.log} checkmarkType="squiggly">
  {' '}
  // $ExpectError toggle me
</StatefulCheckbox>;
...
<FormControl error="Error caption" label="Input label" caption="Input caption">
  Negative
</FormControl>;
...
// Input
<Input onChange={() => ({})} placeholder="Controlled Input" value={'Hey'} />;
...
<Modal onClose={console.log} isOpen={true}>
  ...
  <ModalBody>
    Proin ut dui sed metus pharetra hend rerit vel non mi. Nulla ornare faucibus
    ex, non facilisis nisl. Maecenas aliquet mauris ut tempus.
  </ModalBody>
  <ModalFooter>
...
// Pagination
<Pagination numPages={20} currentPage={1} onPageChange={() => ({})} />;
<Pagination
  numPages={20}
...
// Popover
<Popover isOpen content={<div>hello world</div>}>
  <Block as="span" font="font400">
    Always open
  </Block>
</Popover>;
<StatefulPopover content={<div>hello world</div>} accessibilityType={'tooltip'}>
  <Button>Open</Button>
</StatefulPopover>;
<StatefulPopover
...
<StatefulPopover content={<div>hello world</div>} placement={'topRight'}>
  <Block as="span" font="font400">
    Hover
  </Block>
</StatefulPopover>;
...
<RadioGroup name="radio group" onChange={() => ({})} value={'1'}>
  <Radio value="1">First</Radio>
  <Radio
    value="2"
    description="This is a radio description, it gives a little more in-yo-face context about what this is."
...
<RadioGroup isError name="radio group" onChange={() => ({})} value={'2'}>
  <Radio value="1">First</Radio>
  <Radio value="2">Second</Radio>
  <Radio value="3">Third</Radio>
</RadioGroup>;
...
// Rating
<StarRating value={1} onChange={() => ({})} />;
<EmoticonRating value={3} onChange={() => ({})} />;
<StarRating
  overrides={{
    Invalid: {}, // $ExpectError
  }}
...
<Navigation items={nav} activeItemId={'#level1.1'} onChange={console.info} />;

// Slider
<Slider value={[24]} onChange={console.info} />;
<Slider
...
/>;
<StatefulSlider />;
<Slider disabled value={[24]} onChange={console.info} />;
<Slider value={[10]} min={-300} max={300} step={10} onChange={console.info} />;
<Slider
  value={[2000]}
  min={1000}
  max={8000}
...
<StatefulTabs orientation={'vertical'} initialState={{activeKey: '2'}}>
  <Tab title="Tab Link 1">Tab 1 content</Tab>
  <Tab title="Tab Link 2">Tab 2 content</Tab>
  <Tab title="Tab Link 3">Tab 3 content</Tab>
</StatefulTabs>;
<StatefulTabs
  orientation={TAB_ORIENTATION.vertical}
...
<Tag variant={'solid'} kind={'default'}>
  {' '}
  // $ExpectError With kind
</Tag>;
<Tag />;
