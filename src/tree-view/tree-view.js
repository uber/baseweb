...
export default function TreeView(props: TreeViewPropsT) {
  ...
  const [charsSoFar, setCharsSoFar] = React.useState('');
  const treeItemRefs: {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => setCharsSoFar(''), 500);
  ...
  const onKeyDown = (e: KeyboardEvent, node: TreeNodeT<>) => {
    ...
    switch (e.key) {
      ...
      default:
        e.preventDefault();
        setCharsSoFar(charsSoFar + e.key);
        focusTreeItem(
          getCharMatchId(data, selectedNodeId, charsSoFar + e.key, null, getId),
        );
        break;
    }
  };
