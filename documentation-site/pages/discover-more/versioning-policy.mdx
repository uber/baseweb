...
Breaking changes are inconvenient for everyone, so we thrive to **limit the number of breaking changes by releasing only one major version a year, around September.** To help with the migration, we provide codemods for every major release.
