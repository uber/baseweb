...
An floating map marker is used to mark a location on a map with content.
...
- A floating marker should not change color outside of the default light and dark theme.
...
A floating map marker with a label and start enhancer. Anchor is rendered on the top left of the marker.
...
A floating map marker rendered inside of a [react-map-gl](https://visgl.github.io/react-map-gl/) view. In order to use a floating map marker in react-map-gl, wrap the floating marker with a [`<Marker/>`](https://visgl.github.io/react-map-gl/docs/api-reference/marker) component. You will need to customize the offset of the `<Marker/>` component in order to center the floating map marker's needle at the desired location.
