...
- You can force focus to a Toast with the `autoFocus` prop (on both `Toast` and `Toaster`). Use this with caution and keep in mind that screenreaders may read the Toast's content twice unless you also change the `role` attribute.
