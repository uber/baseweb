...
If a toast with an identical key already exists, that toast's contents will be updated instead of a new toast being created. The autoHide timeout will only reset if `autoHideDuration` value has changed.
