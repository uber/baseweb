...
If a toast with an identical key already exists, that toast's contents will be updated instead of a new toast being created. The autoHide timeout will reset if the `resetAutoHideTimerOnUpdate` prop is set to true on the toaster.
