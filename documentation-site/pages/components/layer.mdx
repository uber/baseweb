...
In order to start leveraging the layers approach add a `LayersManager` to your application. 
It need to be added at the root of the application. If you use [`BaseProvider`](https://baseweb.design/components/base-provider/) 
it will add the `LayersManager` for you. Just make sure you add the `BaseProvider` at the root 
of your application. 

`LayersManager` adds two sibling containers at the root of your application. Your main 
application will be rendered in the first of two containers. You just to pass your application 
root element as a child of `BaseProvider`. The second container is used as a host element 
for layers rendered in the application.

Do not add the `LayersManager` anywhere in the middle of your application since the host element 
for layers added within its context will be positioned at the point where the `LayersManager` is 
added and it will effect the layer stacking order.
**Please note:** if you don't wrap the application in a `LayersManager` all layers will be added 
...
greated then the highest `z-index` value of all top level stacking contexts in your application 
(**note it may be not the highest `z-index` value across your application**).

<Example title="Layers with z-index" path="examples/layer/layer-z-index.js">
  <LayerZIndex />
</Example>
...
<API
  heading="Layer API"
  api={require('!!@uber-web-ui/extract-react-types-loader!../../../src/layer/layer.js')}
/>

<API
  heading="LayersManager API"
  api={require('!!@uber-web-ui/extract-react-types-loader!../../../src/layer/layers-manager.js')}
/>

<API
  heading="Tether API"
  api={require('!!@uber-web-ui/extract-react-types-loader!../../../src/layer/tether.js')}
/>
