...
import SpinnerDeterminate from 'examples/spinner/determinate.js';
...
<Notification overrides={{Body: {style: {width: 'auto'}}}} kind={KIND.info}>
  We have a new, simplified Spinner coming out in v11. You can try it out today
...
We also have a spinner for determinate loading progress, i.e. when you know how far into a task you are.

<Example title="Determinate" path="spinner/determinate.js">
  <SpinnerDeterminate />
</Example>

## Spinner Next

`StyledSpinnerNext` will replace our current `Spinner` in v11 of `baseui`. This will be a breaking change since the APIs between the new and old components are not the same.
...
You can use the new spinner prior to v11 like so:
