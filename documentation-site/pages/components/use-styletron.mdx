...
flow will show an error. To help, baseui exports two utility functions `createStyledThemed` and
`createWithStyleThemed`. These will return their respective functions with a provided theme type.
...
+const styledThemed = createStyledThemed<CustomThemeT>();
+const withStyleThemed = createWithStyleThemed<CustomThemeT>();
+const useStyletronThemed = createUseStyletronThemed<CustomThemeT>();
...
+const First = styledThemed<PropsT>('div', props => {
   return {
...
+const Second = withStyleThemed<PropsT>(First, props => {
   return {
