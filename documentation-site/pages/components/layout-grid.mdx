...
import CustomCssUnitExample from 'examples/layout-grid/custom-css-unit.js';
...
<Example title="Custom css units" path="layout-grid/custom-css-unit.js">
  <CustomCssUnitExample />
</Example>

The grid uses `px` as its default CSS length unit for measuring layout. You can change this, either globally in your theme, or locally via the `unit` prop on `Grid`. If you change the unit, you probably want to update `gridMaxWidth`, `gutters` and `margins` accordingly, as these will pull from your custom unit. For example, when using `em` as your CSS unit, you might update the other values like so:
...
The above change will apply to every grid within the theme's subtree. If you want to override the unit for a single `Grid` instance, you can pass an alternative unit via the `unit` prop:

```
<Grid gridUnit="em" gridMargins={[1, 2, 3]} gridMaxWidth={40}>{/* ... */}</Grid>
```
...
`Grid` and `Cell` also accept an [`overrides`](/guides/understanding-overrides) prop. In the example above, we set the background color of our grid and give every cell in the grid a default `span`. Then we override a single cell (4) to have a higher contrast blue background.

The only way to pass custom styles to `StyledGrid` and `StyledCell` is using `overrides`. It can also be a useful way to pass props to all of your cells in one place (via `Grid.overrides.StyledCell`). Just remember that the props for `StyledGrid` and `StyledCell` have a `$` prefix.

Note, using `overrides` should be a last resort for achieving your desired layout. You can get pretty far by styling an element wrapping `Grid` or immediately inside of a `Cell`. `Grid` and `Cell` are meant to help align your content; generally they should not require styling of their own.
