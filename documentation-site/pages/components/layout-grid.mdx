...
Layout grid uses `px` as a default css unit, you can change that by modifing theme in following manner:
...
If you are using different css unit than pixle, you would also have to add `gutters` and `margins` in same css unit.For example if you are using `em` as a css unit `gutters` and `margins` might have value like below:
