...
Layout grid uses `px` as a default css unit, If you are using different css unit than pixle, you would also have to add `gridMaxWidth`, `gutters` and `margins` in same css unit. For example if you are using `em` as a css unit `gridMaxWidth`, `gutters` and `margins` might have value like below:
