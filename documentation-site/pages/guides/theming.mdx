...
If you are using Flow and want to use a custom theme shape with our various styling utilties, you can create new `styled`, `withStyle`, and `useStyletron` methods which respect your custom theme type.
...
[`styled`](/components/styled) creates a new [Styletron](https://www.styletron.org/) styled component. We provide a version which wraps Styletron’s default `styled` function so that it gets a reference to your theme object. We use this `styled` function to create all of our styled components in Base Web.
