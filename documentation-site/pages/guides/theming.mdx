...
- [Customizing grid-layout](#customizing-grid-layout)
...
Grid layout uses `px` as a default css unit, you can change that by customizing theme.

Please make sure if you are using different css unit than pixle, you would also have to add `gutters` and `margins` in same css unit.

For example if you are using `em` as a css unit `gutters` and `margins` should have value like below:
