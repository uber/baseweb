...
If your application has a custom theme, to properly type check your theme values we will pass the
custom theme type to baseui's `createThemedStyled` function. This will return you a styled function
with your custom theme applied. Doing so saves from needing to import the custom theme type around.
Similar to this, it also works with extending styletron components using `withStyle`. Import
`createThemedWithStyle`, pass your theme type and it returns a `withStyle` function with your the type
...
+const Second = themedWithStyle<PropsT>('div', props => {
   return {
