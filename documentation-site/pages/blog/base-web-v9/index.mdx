...
- Replace direct usage of gray scale (`$theme.colors.mono100` for example) with new values. Make sure to look for references in `Block` and typographic components (`H1`) as well. [Read more 📖](#light-theme-mapping)
...
Unrelated to color but related to themeing: we have updated the default breakpoints in the theme object. The largest breakpoint is now `1136px`. You can set it back to `1280px` in your theme.
