...
If you're using Base Web rest assured that a significant portion of your application will be fine, but accessible components are not an entirely accessible application. You absolutely need to audit your application to be sure all every visitor can successfully use it. Consider the amount of code in your app that does not come from Base Web; it's a lot right? We've done our best to hone the published components, but it's up to you to become familiar with best-practices. A good resource for learning more is the [WAI-ARIA Authoring Practices](https://w3c.github.io/aria-practices/#Listbox) document.
