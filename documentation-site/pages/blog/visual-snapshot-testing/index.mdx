...
Today we are going to talk about Visual Regression Testing. We will consider the benefits of visual regression testing, the availability of existing solutions, and, for the more adventurous folks out there, how to set up a visual snapshot testing pipeline for your own project.
...
When we talk about visual regression testing, we are concerned with preventing unintentional changes to our application's visuals. When we change the styles of our `Button`, how can we be sure we haven't messed up the styles of our `ButtonGroup`? The most common strategy for this sort of assertion is known as a **visual snapshot test**.
